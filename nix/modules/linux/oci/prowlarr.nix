...
in {
  options.modules.linux.oci.services.prowlarr = {
    ...
    baseDir = mkOption {
      description = "Base directory for prowlarr state (mounted at /config).";
      type = types.str;
      example = "/data/apps/prowlarr";
    };

    webPort = mkOption {
      description = "Host port for the prowlarr web UI (forwarded onto gluetun when useGluetun = true).";
      type = types.port;
      default = 9696;
    };
  ...
  in {
    virtualisation.oci-containers.containers.prowlarr = arr.container;
    systemd.services."podman-prowlarr" = arr.serviceConfig;
