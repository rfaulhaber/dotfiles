...
in {
  options.modules.linux.oci.services.bazarr = {
    ...
    baseDir = mkOption {
      description = "Base directory for bazarr state (mounted at /config).";
      type = types.str;
      example = "/data/apps/bazarr";
    };

    webPort = mkOption {
      description = "Host port for the bazarr web UI (forwarded onto gluetun when useGluetun = true).";
      type = types.port;
      default = 6767;
    };
    ...
    mediaDirs = mkOption {
      description = ''
        ...
        Bazarr writes subtitle sidecar files alongside the media, so these need
        to be writable and identical to the paths radarr/sonarr use.
      '';
  ...
  in {
    virtualisation.oci-containers.containers.bazarr = arr.container;
    systemd.services."podman-bazarr" = arr.serviceConfig;
