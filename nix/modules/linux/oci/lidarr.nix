...
with lib; let
  cfg = config.modules.linux.oci.services.lidarr;
  ociLib = config.modules.linux.oci.lib;
in {
  options.modules.linux.oci.services.lidarr = {
    enable = mkEnableOption "Lidarr music collection manager";

    image = mkOption {
      description = "Lidarr container image.";
      type = types.str;
      default = "lscr.io/linuxserver/lidarr:nightly";
    };

    baseDir = mkOption {
      description = "Base directory for lidarr state (mounted at /config).";
      type = types.str;
      example = "/data/apps/lidarr";
    };

    webPort = mkOption {
      description = "Host port for the lidarr web UI (forwarded onto gluetun when useGluetun = true).";
      type = types.port;
      default = 8686;
    };
    ...
    dependsOn = mkOption {
      ...
      type = types.listOf types.str;
      default = [];
      example = ["prowlarr" "transmission" "slskd"];
    };
  ...
  config = mkIf cfg.enable (let
    portMappings = ["${toString cfg.webPort}:8686"];
  ...
  in {
    virtualisation.oci-containers.containers.lidarr = arr.container;
    systemd.services."podman-lidarr" = arr.serviceConfig;
