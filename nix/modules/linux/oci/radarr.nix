...
in {
  options.modules.linux.oci.services.radarr = {
    ...
    baseDir = mkOption {
      description = "Base directory for radarr state (mounted at /config).";
      type = types.str;
      example = "/data/apps/radarr";
    };

    webPort = mkOption {
      description = "Host port for the radarr web UI (forwarded onto gluetun when useGluetun = true).";
      type = types.port;
      default = 7878;
    };
  ...
  in {
    virtualisation.oci-containers.containers.radarr = arr.container;
    systemd.services."podman-radarr" = arr.serviceConfig;
