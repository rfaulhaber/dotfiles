...
in {
  options.modules.services.proxy = {
    ...
    localAliases = mkOption {
      description =
        "Describes mapping between local hostnames and port destinations on the server. This will make each service a subdomain of the local domain name.";
      type = types.attrs;
      example = { "foo.home.lan" = 8089; };
    };
    externalAliases = mkOption {
      description =
        "Describes mapping between publicly available hostnames and port destinations on the server.";
      type = types.attrs;
  ...
  config = mkIf cfg.enable {
    assertions = let isNotBlank = s: s != null && s != "";
    in [
      {
        assertion = (length (attrNames cfg.localAliases)) >= 1
          || (length (attrNames cfg.externalAliases)) >= 1;
        message = "You should specify at least one alias.";
      }
      {
        assertion = ((length (attrNames cfg.externalAliases) >= 1)
          && isNotBlank cfg.defaultEmail) || true;
    ...
    services.nginx = {
      ...
      in localAliases // externalAliases;
    };
    ...
    networking.firewall = {
      ...
      in [ 80 443 ] ++ localPorts ++ externalPorts;
    };
  };
}
